...
parser.add_argument("--vector", type=str, choices=['ad', 'db'], help='If --barcode-type=swim, must specifiy whether ad or db')
...
if barcode_type == 'swim':
	...
	try:
		assert (vector_type == 'ad') or (vector_type == 'db')
	except:
...
def process_read(seq, for_primer_reg, for_rc_primer_reg, rev_primer_reg, rev_rc_primer_reg, use_dlib=False):
		...
		if (for_strand_reverse_primer != for_strand_forward_primer) and ((for_strand_reverse_primer != None) and (for_strand_forward_primer != None)):
